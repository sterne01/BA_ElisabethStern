...
new_lyric_file = "/Users/estern/Documents/BA_Data/BillB+Lyrics.xlsx"
path_f_lyr = "/Users/estern/Documents/BA_Data/F_Bill_Lyr/"
path_m_lyr = "/Users/estern/Documents/BA_Data/M_Bill_Lyr/"
path_NaN = "/Users/estern/Documents/BA_Data/NaN_Bill/"
...
lyr2000 = get_lyrics(path_bill, "2000")

ly_df = pd.DataFrame({"Lyrics": pd.Series(lyr2000)})
og_df = pd.read_excel(path_bill, "2000")
...
for y in all_ws:
    ly = get_lyrics(path_bill, y)
    ly_df = pd.DataFrame({"Lyrics": pd.Series(ly)})
    og_df = pd.read_excel(path_bill, y)
    ...
    gender = new["sex or gender"]
    for x in new["Lyrics"]:
        # falls lyricsgenius keinen Songtext findet (also "empty" in der Lyricsliste steht (s.o.)), wird die Datei
        # einem eigenen Ordner mit leeren Dateien hinzugefügt, sodass diese im Nachhinein noch einmal ausgebessert
        # werden können
        if x == "empty":
